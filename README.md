...
To form the merged constituencies we will use the [Exact Cover](https://en.wikipedia.org/wiki/Exact_cover) problem and [Knuth's Algorithm X](https://en.wikipedia.org/wiki/Knuth%27s_Algorithm_X) to solve it. Note that we will use a PyPi kernel to run Algorithm X as it ran much faster, hence you will need to use the `Dockerfile` in the 'AlgorithmX' folder set it up. Note that the initial analysis and results willbe held in the ANalysis folder which uses a standard Python 3 kernel. 
